...
int main(int argc, char* argv[])
{
    ...
    uint8_t input_buff[BLOCK_SIZE];
    uint8_t output_buff[BLOCK_SIZE];
	...
	switch (mode) {
	    case MODE_ENCRYPT: 
		status = ddd_aes_ref_read_encrypt(output_buff, input_buff, BLOCK_SIZE, key_k, key_l, tweak, TWEAKSIZE_BYTES);
		break;
	    case MODE_DECRYPT: 
		status = ddd_aes_ref_read_decrypt(output_buff, input_buff, BLOCK_SIZE, key_k, key_l, tweak, TWEAKSIZE_BYTES);
		break;
	    default:
		status = -1;
	}
